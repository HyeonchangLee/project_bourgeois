        ...
        if (filter.hasClass('kf_paymentBox'))
         {
          ...
          // 렌터카, 숙박 예외처리
          if( filter.parents('.k1_schedule_search').hasClass('rentcar') || filter.parents('.k1_schedule_search').hasClass('stay') ) {
            headerH2 = 0;
          }
          ...
          if (!filter.hasClass('filterFoot')) {
            if (sorting.hasClass('fixed') && filter.height() < $('#contents').height() ){
              filter.css({ position: 'fixed', top: '' });
            } else {
              filter.css({ position: 'absolute', top: '0' });
            }
          }
